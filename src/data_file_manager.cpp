...
Fl_Window* Data_File_Manager::edit_labels_window = nullptr;
Fl_Check_Browser* Data_File_Manager::edit_labels_widget = nullptr;
...
int Data_File_Manager::load_data_file( string in_file_spec)
{
  input_filespec( in_file_spec );
...
int Data_File_Manager::read_table_from_fits_file()
{
  ...
  npoints = int(local_nrows);
  nDataRows_ = int(local_nrows);
  nvars = local_ncols;
  blitz::Range NPTS( 0, npoints-1);
...
int Data_File_Manager::findOutputFile()
{
  ...
  // char *cOutFileSpec =
  //   Vp_File_Chooser( "write ASCII output to file", nullptr, nullptr, 0);
...
int Data_File_Manager::save_data_file( string in_outFileSpec)
{
  output_filespec( in_outFileSpec);
...
void Data_File_Manager::edit_column_info( Fl_Widget *o, void * user_data = nullptr)
{
	UNUSED(user_data);
  // DIAGNOSTIC
